...
const LINE_DEFS = [
  ...
  { id: 'hankyu_kobe', operator: '阪急', ekidataId: 34001, name: '神戸本線', expectedBase: 5, startName: '梅田', endName: '園田' },
  { id: 'hankyu_takarazuka', operator: '阪急', ekidataId: 34002, name: '宝塚本線', expectedBase: 5, startName: '梅田', endName: '庄内' },
  { id: 'hankyu_kyoto', operator: '阪急', ekidataId: 34003, name: '京都本線', expectedBase: 8, startName: '正雀', endName: '梅田' },
  { id: 'hankyu_senri', operator: '阪急', ekidataId: 34008, name: '千里線', expectedBase: 5, startName: '吹田', endName: '天神橋筋六丁目' },
  { id: 'hanshin_main', operator: '阪神', ekidataId: 35001, name: '本線', expectedBase: 7, startName: '梅田', endName: '杭瀬' },
  { id: 'hanshin_namba', operator: '阪神', ekidataId: 35002, name: '阪神なんば線', expectedBase: 10, startName: '大阪難波', endName: '大物' },
  { id: 'keihan_main', operator: '京阪', ekidataId: 33001, name: '京阪本線', expectedBase: 9, startName: '滝井', endName: '淀屋橋' },
  { id: 'keihan_nakanoshima', operator: '京阪', ekidataId: 33008, name: '中之島線', expectedBase: 5 },
  { id: 'kintetsu_namba', operator: '近鉄', ekidataId: 31001, name: '難波線', expectedBase: 3 },
  { id: 'kintetsu_osaka', operator: '近鉄', ekidataId: 31005, name: '大阪線', expectedBase: 4, startName: '大阪上本町', endName: '布施' },
  { id: 'kintetsu_minamiosaka', operator: '近鉄', ekidataId: 31003, name: '南大阪線', expectedBase: 7, startName: '大阪阿部野橋', endName: '河内天美' },
  { id: 'nankai_main', operator: '南海', ekidataId: 32001, name: '南海本線', expectedBase: 8, startName: '難波', endName: '七道' },
  { id: 'nankai_koya', operator: '南海', ekidataId: 32007, name: '高野線', expectedBase: 11, startName: '難波', endName: '浅香山' },
];
...
function addStation(gid, name, lat, lng, lineId) {
  if (SHARED_BY_NAME.has(name)) {
    const shared = [...byId.values()].find((node) => node.name === name);
    if (shared) {
      if (!shared.lines.includes(lineId)) shared.lines.push(lineId);
      return shared.id;
    }
  }
  const key = gid + '|' + name;
...
for (const node of byId.values()) {
  ...
  if (node.ridership == null) {
    const operatorWithNote = node.lines.map((lid) => operatorByLine.get(lid)).find((op) => rider.operatorNotes?.[op]);
    if (operatorWithNote) {
      node.ridership = rider.operatorNotes[operatorWithNote];
    }
  }
}
...
const meta = {
  ...
  sources: [
    ...
    { name: '阪急電鉄 路線図・駅情報', license: '公式サイト', note: '阪急4路線の駅構成確認(2026-07-11取得)' },
    { name: '阪神電車 駅情報・路線図', license: '公式サイト', note: '阪神2路線の駅構成確認(2026-07-11取得)' },
    { name: '京阪電車 駅・路線図', license: '公式サイト', note: '京阪2路線の駅構成確認(2026-07-11取得)' },
    { name: '近畿日本鉄道 路線図・駅情報', license: '公式サイト', note: '近鉄3路線の駅構成確認(2026-07-11取得)' },
    { name: '南海電鉄 路線図・駅情報', license: '公式サイト', note: '南海2路線の駅構成確認(2026-07-11取得)' },
  ...
  notes: [
    ...
    'M3私鉄路線は大阪市内+市境1駅を基本に収録。近鉄奈良線は布施1駅のみで単独路線として成立しないためSprint4では非収録',
    '私鉄の駅別乗降人員はSprint4時点で未投入。既存のMetro/JR共有駅は既存出典値を表示し、私鉄単独駅は注記表示',
